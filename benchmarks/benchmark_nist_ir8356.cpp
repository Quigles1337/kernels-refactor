...
struct BenchmarkStats {
  ...
  BenchmarkStats()
      : min_val(0), mean(0), max_val(0), std_dev(0), count(0),
        status("UNKNOWN") {}
};

BenchmarkStats compute_stats(const std::vector<double> &data,
                             const std::string &pass_condition = "PASS") {
...
void print_results_header() {
  std::cout << "\n╔════════════════════════════════════════════════════════════"
               "═══════════════╗\n";
  std::cout << "║           NIST IR 8356 Quantum Computing Metrics Results "
               "Matrix           ║\n";
  std::cout << "╚══════════════════════════════════════════════════════════════"
               "═════════════╝\n";
}

void print_metric_row(const std::string &metric, const BenchmarkStats &stats,
                      const std::string &unit) {
  std::cout << "│ " << std::left << std::setw(32) << metric;
  std::cout << "│ " << std::right << std::setw(10) << std::fixed
            << std::setprecision(4) << stats.min_val << " " << unit;
  std::cout << "│ " << std::right << std::setw(10) << std::fixed
            << std::setprecision(4) << stats.mean << " " << unit;
  std::cout << "│ " << std::right << std::setw(10) << std::fixed
            << std::setprecision(4) << stats.max_val << " " << unit;
...
ProcessSpawnResults benchmark_process_spawning() {
  ...
  std::cout << "╚═══════════════════════════════════════════════════╝\n";
  std::cout << "\nEvaluating system capacity to spawn quantum processes under "
               "load...\n";
  ...
  for (size_t n : test_counts) {
    ...
    std::cout << "    Spawn time: " << std::setprecision(4) << duration_ms
              << " ms\n";
    std::cout << "    Per-process: " << std::setprecision(4) << per_process_us
              << " μs\n";
    std::cout << "    Coherence: " << std::setprecision(6) << avg_coherence
              << "\n";
    std::cout << "    Success rate: " << std::setprecision(2)
              << (success_rate * 100) << "%\n";
  }
...
SchedulingResults benchmark_scheduling_throughput() {
  ...
  for (size_t n_ops : op_counts) {
    ...
    avg_coherence /= processes.size();
    double coherence_rate =
        static_cast<double>(coherent_count) / processes.size();
    ...
    std::cout << "    Cycle time: " << std::setprecision(4) << cycle_time_us
              << " μs\n";
    std::cout << "    Throughput: " << std::setprecision(2) << ops_per_second
              << " ops/s\n";
    std::cout << "    Coherence preservation: " << std::setprecision(2)
              << (coherence_rate * 100) << "%\n";
  }
...
MemoryBenchmarkResults benchmark_memory_scalability() {
  ...
  for (size_t n_addresses : address_sizes) {
    ...
    auto write_duration_ns =
        std::chrono::duration<double, std::nano>(write_end - write_start)
            .count();
    ...
    double uniformity_score =
        1.0 /
        (1.0 + uniformity_variance / (expected_per_bank * expected_per_bank));
    ...
    std::cout << "    Avg write: " << std::setprecision(4) << avg_write_ns
              << " ns\n";
    std::cout << "    Avg read: " << std::setprecision(4) << avg_read_ns
              << " ns\n";
    std::cout << "    Bank uniformity: " << std::setprecision(4)
              << uniformity_score << "\n";
...
CoherenceScaleResults benchmark_coherence_at_scale() {
  ...
  for (size_t n : scale_levels) {
    ...
    double avg_coh = std::accumulate(coherence_samples.begin(),
                                     coherence_samples.end(), 0.0) /
                     coherence_samples.size();
    double min_coh =
        *std::min_element(coherence_samples.begin(), coherence_samples.end());
    ...
    std::cout << "    Conservation error: " << std::setprecision(3)
              << std::scientific << conservation_err << std::fixed << "\n";
  }
...
void print_comprehensive_results(const ProcessSpawnResults &spawn_res,
  ...
  auto coherence_spawn_stats = compute_stats(
      spawn_res.coherence_values,
      spawn_res.coherence_values.empty()
          ? "FAIL"
          : (*std::min_element(spawn_res.coherence_values.begin(),
                               spawn_res.coherence_values.end()) > 0.99
                 ? "PASS"
                 : "WARN"));
  ...
  auto throughput_stats =
      compute_stats(sched_res.operations_per_second, "PASS");
  ...
  auto uniformity_stats = compute_stats(
      mem_res.bank_uniformity,
      mem_res.bank_uniformity.empty()
          ? "FAIL"
          : (*std::min_element(mem_res.bank_uniformity.begin(),
                               mem_res.bank_uniformity.end()) > 0.95
                 ? "PASS"
                 : "WARN"));
  ...
  auto coh_avg_stats =
      compute_stats(coh_res.avg_coherence,
                    coh_res.avg_coherence.empty()
                        ? "FAIL"
                        : (*std::min_element(coh_res.avg_coherence.begin(),
                                             coh_res.avg_coherence.end()) > 0.99
                               ? "PASS"
                               : "WARN"));
  ...
  auto conservation_stats =
      compute_stats(coh_res.conservation_error,
                    coh_res.conservation_error.empty()
                        ? "FAIL"
                        : (*std::max_element(coh_res.conservation_error.begin(),
                                             coh_res.conservation_error.end()) <
                                   CONSERVATION_TOL
                               ? "PASS"
                               : "WARN"));
...
int main(int argc, char *argv[]) {
  std::cout << "╔══════════════════════════════════════════════════════════════"
               "═════════════╗\n";
  std::cout << "║       NIST IR 8356 Quantum Computing Metrics Benchmarking "
               "Suite           ║\n";
  std::cout << "║                    Quantum Kernel Scalability Evaluation     "
               "             ║\n";
  std::cout << "╚══════════════════════════════════════════════════════════════"
               "═════════════╝\n";
  std::cout << "\nThis benchmark suite evaluates quantum kernel scalability "
               "against\n";
  std::cout
      << "metrics defined in NIST IR 8356 for quantum computing systems.\n";
