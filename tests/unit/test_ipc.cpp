...
void test_message_structure() {
  ...
  test_assert(std::abs(msg.payload - Cx{0.5, 0.5}) < 1e-9,
              "Payload stored correctly");
  test_assert(std::abs(msg.sender_coherence - 0.95) < 1e-9,
              "Coherence stored correctly");
}
...
void test_payload_bounds() {
  ...
  test_assert(large_norm <= MAX_PAYLOAD_NORM,
              "Large valid payload within bounds");
  ...
  test_assert(excessive_norm > MAX_PAYLOAD_NORM,
              "Excessive payload exceeds bounds");
...
void test_cycle_alignment() {
  ...
  test_assert(should_deliver_match,
              "Exact cycle position match allows delivery");
  ...
  test_assert(should_not_deliver_zero,
              "Position 0 does not bypass cycle alignment");

  // Test case 3: Different position
  uint8_t receiver_pos_diff = 5;
  bool should_wait =
      (receiver_pos_diff != sender_pos && receiver_pos_diff != 0);
...
void test_channel_management() {
  ...
  struct ChannelId {
    ...
    bool operator==(const ChannelId &other) const {
      return sender_pid == other.sender_pid &&
             receiver_pid == other.receiver_pid;
    }
  };
  ...
  test_assert(ch1.sender_pid != ch3.sender_pid ||
                  ch1.receiver_pid != ch3.receiver_pid,
...
void test_metadata_integrity() {
  ...
  test_assert(std::abs(stored.data - original.data) < 1e-15,
              "Payload preserved");
  test_assert(std::abs(stored.coherence - original.coherence) < 1e-15,
              "Coherence preserved");
}
...
void test_coherence_boundaries() {
  ...
  std::vector<double> coherences = {0.0,  0.49, 0.5,  0.69, 0.7,
                                    0.89, 0.9,  0.99, 1.0};
  ...
  for (double threshold : thresholds) {
    for (double coherence : coherences) {
      ...
      test_assert(expected_result == (coherence >= threshold),
                  "Threshold " + std::to_string(threshold) + " vs coherence " +
                      std::to_string(coherence));
    }
  }
}
...
void test_chiral_linear_domain() {
  ...
  // Case C: gate produces non-trivial output (rotation does move the state)
  {
    ...
    test_assert(std::abs(state.beta - original_beta) > COHERENCE_TOL,
...
void test_chiral_nonlinear_domain() {
  ...
  test_assert(diff_zero < COHERENCE_TOL,
...
void test_chiral_ipc_compatibility() {
  std::cout << "\n╔═══ Test 15: Chiral Gate — IPC Coherence Scheduling "
               "Compatibility ═══╗\n";
  ...
  for (int i = 0; i < 8; ++i) {
    double c = coherence(state.radius());
  ...
  QState balanced;
  for (int i = 0; i < 8; ++i) {
    double c = coherence(balanced.radius());
    test_assert(c >= 0.0 && c <= 1.0 + COHERENCE_TOL,
  ...
  // Verify coherence_kick_strength=0.0 (process default) produces linear
  // behaviour This mirrors Process::tick() with zero kick — must equal
  // state.tick()
  {
...
void test_classical_layer_compatibility() {
  ...
  test_assert(
      std::abs(mag_after - mag_before) < COHERENCE_TOL,
